...
public class GemBodyLayer<T extends GeoAnimatable, R extends GemRenderState> extends GemTintedTextureLayer<T, R> {
    private final String gemName;

    public GemBodyLayer(GeoRenderer<T, Void, R> renderer, String gemName, int order) {
    ...
    protected @Nullable Identifier getTextureResource(R state) {
        return Identifier.fromNamespaceAndPath(GemColony.MOD_ID, "textures/entity/gems/" + gemName + "/" + gemName + "_skin.png");
    }
