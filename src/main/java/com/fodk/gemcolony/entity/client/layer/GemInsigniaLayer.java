...
public class GemInsigniaLayer<T extends GeoAnimatable, R extends GemRenderState> extends GemTintedTextureLayer<T, R> {
    private final String gemName;

    public GemInsigniaLayer(GeoRenderer<T, Void, R> renderer, String gemName, int order) {
    ...
    protected @Nullable Identifier getTextureResource(R state) {
        if (state.insignia < 0) return null;
        return Identifier.fromNamespaceAndPath(GemColony.MOD_ID, "textures/entity/gems/" + gemName + "/insignia/insignia_" + state.insignia + ".png");
    }

    @Override
    protected int getTintColor(R state) {
        return state.insigniaColor;
    }
